...
```{r}
date.search <- format(Sys.time(), "%Y%m%d")
URL <- "https://www.mangoplate.com/search/%EC%97%AC%EC%9D%98%EB%8F%84?keyword=%EC%97%AC%EC%9D%98%EB%8F%84&page=1"
...
tab <- cbind(star, title, link) %>% as_tibble()

df <- tab %>%
  mutate(title.link = cell_spec(title, "html", link = link, color="#062872")) %>%
  select(star, title.link)

names(df) <- c("Star", "Headline")
